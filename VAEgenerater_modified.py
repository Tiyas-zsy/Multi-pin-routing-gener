...
class VAEgen(nn.Module):
    ...
    def __init__(self, hiddens:list =[32, 64], latent_dim=512) -> None:
        super().__init__()
    ...
    def decoder(self,z):
        ...
        modules = []
        in_channels = 128
        decoder_projection = nn.Linear(self.latent_dim, 128*32*32).to('cuda:0')
        ...
        decoder = nn.Sequential(*modules).to('cuda:0')
        x = decoder_projection(z)
        x = torch.reshape(x, (-1, *decoder_input_chw)).cuda()
        decoded = decoder(x)
        return decoded

...
class function():
    ...
    def to_image(self,input,num):
        ...
        name = 'E:/EDA/code/Multi_pin_routing_solver/mydata/gendata0/' + str(num) + '.png'
...
g_name = 'E:/EDA/code/Multi_pin_routing_solver/mydata/gendata0/Gen.pth'
d_name = 'E:/EDA/code/Multi_pin_routing_solver/mydata/gendata0/disc.pth'
use_gpu =1
if use_gpu:
...
for epoch in range(num_epoch):
    for i,mini_batch in enumerate(dataloader):
        ...
        vae_input = np.zeros((16,1,128,128),dtype=np.float32)
        vae_input[:,0,:,:] = mini_batch[:,0,:,:]
        if use_gpu:
            t_vae_input = torch.from_numpy(vae_input).to("cuda")
        else:
            t_vae_input = torch.from_numpy(vae_input)

        #routed labels
        a_labels= np.zeros((16,1,128,128),dtype=np.float32)
        a_labels[:,0,:,:] = mini_batch[:,2,:,:]
        if use_gpu:
            t_labels = torch.from_numpy(a_labels).to("cuda")
        else:
            t_labels = torch.from_numpy(a_labels)
        ...
        p2m = pred_images
        torch.where(p2m > 0.5, p2m + 0.5,p2m - 0.5)
        ...
        A = discriminator(pred_images)#for debug
        org_loss = loss_fn(discriminator(pred_images),labels_one)#数量级：100
        g_loss = generator.loss_fnn(lr_dist,p2m,t_labels) 
        #+loss_fn(discriminator(pred_images),labels_one)
        loss = g_loss.cpu()
        loss_ = str(loss.data.numpy())
        with open('E:/EDA/code/Multi_pin_routing_solver/mydata/gendata0/loss.txt', 'a') as f:
            f.write(str(i + epoch*150))
            f.write(' ')
            f.write(loss_)
            f.write(' \r\n')
        g_loss.backward()
